...
/**
 * @brief Adds two linked lists together, dropping values with 
 * indices greater than is common between the two lists.
 * 
 * @param list1 The first list, the one being added to.
 * @param list2 The second list, the one being added.
 */
void add_lists(Node *list1, Node *list2)
{
  // create new pointers to iterate through the lists
  Node *c1 = list1, *c2 = list2;

  // for each value, sum the values and insert the sum into the new list
...
/**
 * @brief Create a duplicate of a linked list at a different memory location.
 * 
 * @param head The head of the list to duplicate.
 * @return Node* The head of the duplicate list.
 */
Node *duplicate_list(Node *head)
{
  // create a new list
  ...
  // iterate through the list and insert each value into the new list
  while (head != NULL) {
    new_list = insert_at_head(new_list, head->value);
    head = head->next;
  }
