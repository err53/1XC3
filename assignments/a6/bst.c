...
/**
 * @brief Recursive helper function for array_of_sorted_keys
 * 
 * @param node The current node in the tree
 * @param array The array to store the sorted keys in
 * @param current_index The current index in the array
 */
void build_array(bstNode *node, int *array, int *current_index)
{
  // Base case
  ...
  // Recursive case
  // Recursively call build_array on the left child
  build_array(node->left_child, array, current_index);
  // Store the current node's key in the array and increment the index
  array[*current_index] = node->key;
  *current_index = *current_index + 1;
  // Recursively call build_array on the right child
  build_array(node->right_child, array, current_index);
}

/**
 * @brief Converts a BST into an array of sorted keys
 * 
 * @param node The root of the BST
 * @return int* The array of sorted keys
 */
int *array_of_sorted_keys(bstNode *node)
{
  ...
  // Recursively call build_array on the root node
  build_array(node, array, &current_index);
